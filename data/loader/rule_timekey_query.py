...
메타 SQL (data/sql/, SQL_JSON_MAP 제외):
    ...
    rule_timekey_recent.sql  – 현재시각-N일 ~ 현재시각 범위
...
def fetch_recent_rule_timekeys(
    ...
    *,
    ...
    """DB 최근 N일 범위 RULE_TIMEKEY (오름차순). sql 없으면 None."""
    if not use_db_rule_timekey():
        return None
    ...
    fac_id = validate_path_segment(fac_id, "FAC_ID")
    to_key = normalize_rule_timekey(rule_timekey_now())
    to_dt = datetime.strptime(to_key, RULE_TIMEKEY_FMT)
    from_key = (to_dt - timedelta(days=prevdays)).strftime(RULE_TIMEKEY_FMT)
    rows = execute_meta_sql(
        RULE_TIMEKEY_RECENT_SQL,
        {
            "FAC_ID": fac_id,
            "FROM_RULE_TIMEKEY": from_key,
            "TO_RULE_TIMEKEY": to_key,
        },
        db_registry=db_registry,
    )
