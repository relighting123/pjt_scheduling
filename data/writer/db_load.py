...
from data.db_registry import DbRegistry, parse_sql_db_alias, parse_per_table_db_aliases
...
def _load_per_table_aliases(default_alias: Optional[str]) -> dict:
    """data/sql/rts_output_tables.sql 의 테이블별 -- @db: 헤더 → {TABLE: alias}.

    DDL 파일이 없으면(아직 db-load --ddl-only 를 안 돌린 경우 등) 빈 dict를
    반환해 모든 테이블이 기존처럼 하나의 db_alias 를 그대로 쓰게 한다.
    """
    try:
        ddl_path = _resolve_ddl_path()
    except FileNotFoundError:
        return {}
    return parse_per_table_db_aliases(
        ddl_path.read_text(encoding="utf-8"),
        default_alias=default_alias or "main",
    )
...
def load_output_sql_files(
    ...
    *,
    ...
    테이블별 대상 DB는 data/sql/rts_output_tables.sql 의 테이블별 -- @db:
    헤더(parse_per_table_db_aliases)로 지정할 수 있다 — 헤더가 없으면(또는
    DDL 파일 자체가 없으면) 모든 테이블이 db_alias(또는 registry 기본 alias)
    하나만 그대로 쓴다(기존 동작과 동일).
    """
    ...
    registry = DbRegistry()
    table_aliases = _load_per_table_aliases(db_alias or registry.default_alias)
    executed: List[Path] = []
