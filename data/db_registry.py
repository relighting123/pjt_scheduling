...
Thick 모드 (.env):
    ORACLE_THICK_MODE=1
    ORACLE_CLIENT_LIB_DIR=/path/to/instantclient   # Linux/Windows
    ORACLE_CONFIG_DIR=/path/to/tnsadmin            # 선택 (tnsnames.ora)
"""
...
def oracle_thick_mode_enabled() -> bool:
...
def init_oracle_client_if_needed() -> bool:
    """
    oracledb thick 모드 초기화 (프로세스당 1회).

    Returns:
        thick 모드로 초기화했으면 True, thin 모드면 False
    """
    global _oracle_client_initialized
    if _oracle_client_initialized:
        return oracle_thick_mode_enabled()
    if not oracle_thick_mode_enabled():
        return False
    ...
    config_dir = os.environ.get("ORACLE_CONFIG_DIR", "").strip()
    if lib_dir:
...
def diagnose_db_config(
    ...
    return {
        ...
        "issues": issues,
        "notes": notes,
        "oracle_mode": "thick" if oracle_thick_mode_enabled() else "thin",
        "ok": not issues,
    }
...
def print_db_config_report(report: dict) -> None:
    ...
    print(f"oracle mode: {report.get('oracle_mode', 'thin')}")
