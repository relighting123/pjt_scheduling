...
-- ── 버킷(PPK/OPER) 후보 이력: 배정 시점에 고를 수 있었던 다른 후보 전부(선택된 것 포함) ──
-- CARRIER_ID로 RTS_RSLT_MAS/HIS의 같은 FAC_ID+RULE_TIMEKEY+EQP_ID+CARRIER_ID 행과 연결된다.
-- INSERT only(이력 누적) — 옵션(CONFIG.env.candidate_output_enabled)이 꺼지면 비어서 출력됨.
CREATE TABLE RTS_RSLT_CANDIDATE_HIS (
    FAC_ID              VARCHAR2(32)   NOT NULL,
    RULE_TIMEKEY        VARCHAR2(14)   NOT NULL,
    EXEC_TIMEKEY        VARCHAR2(14)   NOT NULL,
    EQP_ID              VARCHAR2(64)   NOT NULL,
    ...
    CRT_USER_ID         VARCHAR2(32)   DEFAULT 'RTS' NOT NULL,
    CRT_TM              TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
    CONSTRAINT PK_RTS_RSLT_CANDIDATE_HIS PRIMARY KEY
        (FAC_ID, EXEC_TIMEKEY, RULE_TIMEKEY, EQP_ID, CARRIER_ID, CANDIDATE_PPK, CANDIDATE_OPER_ID)
);

CREATE INDEX IX_RTS_RSLT_CANDIDATE_HIS_LINK
    ON RTS_RSLT_CANDIDATE_HIS (FAC_ID, RULE_TIMEKEY, EQP_ID, CARRIER_ID);
