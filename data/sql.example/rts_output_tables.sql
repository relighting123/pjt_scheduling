...
-- ── 버킷(PPK/OPER) 선택 후보 추적: 배정 시점에 고를 수 있었던 다른 후보 전부(선택된 것 포함) ──
-- CARRIER_ID로 RTS_RSLT_MAS/HIS의 같은 FAC_ID+RULE_TIMEKEY+EQP_ID+CARRIER_ID 행과 연결된다.
-- RTS_EQPCONVPLAN_INF/HIS와 동일 패턴: INF는 동일 FAC_ID+RULE_TIMEKEY 기존 행만 DELETE 후
-- INSERT(같은 회차 재실행 시 중복 방지), HIS는 EXEC_TIMEKEY가 PK에 포함되어 회차마다 누적.
-- 옵션(CONFIG.env.trace_output_enabled)이 꺼지면 두 테이블 모두 비어서 출력된다.
CREATE TABLE RTS_TRACE_INF (
    FAC_ID              VARCHAR2(32)   NOT NULL,
    RULE_TIMEKEY        VARCHAR2(14)   NOT NULL,
    EQP_ID              VARCHAR2(64)   NOT NULL,
    CARRIER_ID          VARCHAR2(64)   NOT NULL,   -- 이 후보 그룹이 속한 배정(RTS_RSLT_MAS/HIS.CARRIER_ID)
    CANDIDATE_PPK       VARCHAR2(64)   NOT NULL,
    CANDIDATE_OPER_ID   VARCHAR2(64)   NOT NULL,
    IS_SELECTED         CHAR(1)        DEFAULT 'N' NOT NULL,  -- 실제로 선택된 후보면 Y
    ...
    CRT_USER_ID         VARCHAR2(32)   DEFAULT 'RTS' NOT NULL,
    CRT_TM              TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
    EXEC_TIMEKEY        VARCHAR2(14)   NOT NULL,   -- 생성 시각(NOT NULL), PK엔 없음 — 재실행마다 최신값으로 갱신
