...
STATE_WALKTHROUGH = [
    ...
    },
    {
        ...
        "lines": [
            ...
            "2294  feats[oi, pi, vmis, 11] = min(cov11 / need11, 2.0) / 2.0",
        ],
        "calc": [
    ...
    },
    {
        ...
        "lines": [
            "2320  eqp_local = np.zeros(4, dtype=np.float32)",
            "2321  current_eqp_id = self._current_eqp",
            "2322  if current_eqp_id and current_eqp_id in self.eqps:",
            "2323      eqp_obj = self.eqps[current_eqp_id]",
            "2324      eqp_local[2] = encode_normalized(eqp_obj.prev_prod, prod_idx, P)",
            "2325      eqp_local[3] = encode_normalized(eqp_obj.prev_oper, oper_idx, O)",
            "2330      max_avoidable = 0.0",
            "2331      for flat in self.get_feasible_ppk_oper(current_eqp_id):",
            "2334          if self._would_need_conversion(current_eqp_id, lc, tp):",
            "2335              eqp_local[0] = 1.0",
            "2341      eqp_local[1] = max_avoidable",
        ],
        "calc": [
