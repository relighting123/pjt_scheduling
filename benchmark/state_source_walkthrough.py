...
STATE_WALKTHROUGH = [
    {
        ...
        "lines": [
            "2356  group_global[0] = min(self.current_time / max(self.sim_end, 1), 1.0)",
            "2357  group_global[1] = min(",
            "2358      max(self.soft_cutoff - self.current_time, 0) / max(self.soft_cutoff, 1), 1.0,",
            "2359  )",
        ],
        "calc": [
    ...
    },
    {
        ...
        "lines": [
            "2323  initial_lot_count = max(len(data[\"lots\"]), 1)",
            "2324  total_plan = max(",
            "2325      sum(p[\"d0_plan_qty\"] for p in data.get(\"plan\", []) if p.get(\"d0_plan_qty\", 0) > 0),",
            "2326      0,",
            "2327  )",
            "  ...",
            "2360  group_global[2] = min(len(self.lot_pool) / initial_lot_count, 1.0)",
            "2361  produced = sum(self.stats[\"completed_qty\"].values())",
            "2362  if total_plan > 0:",
            "2363      group_global[3] = min(produced / total_plan, 1.0)",
            "2364  else:",
            "2365      group_global[3] = min(produced / max(self._initial_wip_total, 1), 1.0)",
        ],
        "calc": [
    ...
    },
    {
        ...
        "lines": [
            "2366  conv_eqps = 0",
            "2367  for eqp_id, eqp in self.eqps.items():",
            "2368      rem = max(eqp.free_at - self.current_time, 0)",
            "2369      if rem > 0 and eqp.status == \"idle\" and eqp.prev_lot_cd is not None:",
            "2370          conv_eqps += 1",
            "2371  group_global[4] = conv_eqps / max(len(self.eqps), 1)",
            "2372  group_global[5] = min(self._tool_tracker.utilization(), 1.0)",
        ],
        "calc": [
    ...
    },
    {
        ...
        "title": "ch0 wip_ratio_total  ·  ch1 wip_ratio_ppk",
        "lines": [
            ...
            "  ...",
            "2279  feats[oi, pi, vmis, 0] = wip_q / total_wip",
            "2280  feats[oi, pi, vmis, 1] = wip_q / ppk_total",
        ],
        "calc": [
        ...
        "result": "ch0 = 0.0857   /   ch1 = 0.0968",
        "note": "분모가 다르다: ch0은 '팹 전체 WIP', ch1은 '같은 PPK001 안에서의 WIP' — 같은 wip_q라도 두 관점을 동시에 준다.",
    },
    {
        "group": "버킷 (Bucket) — OPER001 vs OPER002 버킷 비교",
        "title": "ch2 prev_takt  ·  ch3 post_takt",
        "lines": [
    ...
    },
    {
        ...
        "lines": [
            ...
            "  ...",
            "2280  feats[oi, pi, vmis, 4] = vsts / max_arrange_st",
        ],
        "calc": [
    ...
    },
    {
        ...
        "lines": [
            ...
            "  ...",
            "2285  feats[oi, pi, vmis, 5] = urgency",
        ],
        "calc": [
    ...
    },
    {
        ...
        "lines": [
            ...
            "  ...",
            "2286  feats[oi, pi, vmis, 6] = lc_enc",
            "2287  feats[oi, pi, vmis, 7] = tp_enc",
        ],
        "calc": [
            "이 버킷의 lc=\"A\", tp=None",
        ...
        "result": "wip_lot_cd (ch6) = 1.0   /   wip_temp (ch7) = 0.0",
        "note": "ch7의 0.0은 'TEMP 조건이 아예 없다'는 뜻이지, 오류나 결측이 아니다 — encode_normalized(None,...)의 정상 동작.",
    },
    {
        "group": "버킷 (Bucket) — current_eqp=EQP002 기준",
        "title": "ch8 needs_conversion  ·  ch9 tool_can_assign",
        "lines": [
        ...
        "result": "needs_conversion (ch8) = 1.0   /   tool_can_assign (ch9) = 1.0",
        "note": "이 두 채널은 current_eqp가 속한 (op,model) 한 칸에만 기록되고, 나머지 칸은 0으로 남는다.",
    },
    {
        "group": "버킷 (Bucket) — (PPK001, OPER002) 기준",
        "title": "ch10 achievable_ratio",
        "lines": [
            ...
            "  ...",
            "2288  feats[oi, pi, vmis, 10] = achievable_ratio",
        ],
        "calc": [
            "plan_qty=60, done=10",
        ...
        "result": "achievable_ratio (ch10) = 1.0",
        "note": "상류 재공(140장)이 충분해서 achievable이 plan_qty(60)에 그대로 캡핑됨 — 재공 부족 걱정이 없는 상태.",
    },
    {
        "group": "버킷 (Bucket) — (PPK001, OPER002) 기준",
        "title": "ch11 projected_cover_ratio",
        "lines": [
            "2277  # 채널 11: 투영 커버 비율 — 다른 장비(current_eqp 제외)가 이 버킷을",
            "2278  # 하루 끝까지 덮는 양 / 남은 필요량. 1에 가까울수록 '이미 덮임' → 회피 유도.",
            "2279  done11 = completed.get((ppk, op), 0)",
            "2280  need11 = max(self._achievable_qty(ppk, op) - done11, 1.0)",
            "2281  cov11 = self._bucket_projected_cover(ppk, op, exclude_eqp=current_eqp)",
            "2282  feats[oi, pi, vmis, 11] = min(cov11 / need11, 2.0) / 2.0",
        ],
        "calc": [
        ...
        "result": "projected_cover_ratio (ch11) = 0.72",
    ...
    {
        "group": "버킷 (Bucket) — (PPK001, OPER002) 기준",
        "title": "ch12 starve_time_norm",
        "lines": [
            "2282  # 채널 12: starve_time_normalized (재공 소진 시간)",
            "2283  consume_rate = self._oper_capacity_per_min(ppk, op)",
            "2284  supply_rate = self._oper_supply_rate(ppk, op)",
            "2285  if consume_rate > supply_rate:",
            "2286      net_rate = consume_rate - supply_rate",
            "2287      starve_time = wip_q / net_rate",
            "2288      starve_time_norm = min(starve_time / T_avail, 1.0)",
            "2289  else:",
            "2290      starve_time_norm = 1.0",
            "2291  feats[oi, pi, vmis, 12] = starve_time_norm",
        ],
        "calc": [
            "wip_q = wip_po[(PPK001,OPER002)] = 15",
            "consume_rate: EQP001(M1) st=5, EQP002(M1) st=5, EQP003(M1) st=5 (가정) → 3 * (1/5) = 0.6 매/분",
            "supply_rate: 선행 OPER001의 capa = EQP001(M1) st=2, EQP002(M1) st=2 → 2 * (1/2) = 1.0 매/분 (wip존재)",
            "consume_rate(0.6) <= supply_rate(1.0) → starve_time_norm = 1.0 (재공이 마르지 않음)",
        ],
        "result": "starve_time_norm (ch12) = 1.0",
        "note": "이 공정(OPER002)은 공급 속도가 소비 속도보다 빨라 재공이 마르지 않기 때문에 소진 시간 비율이 최댓값인 1.0이 된다.",
    },
    {
        "group": "현재 설비 (EQP local) — current_eqp=EQP002",
        "title": "eqp[0~1] needs_conversion · avoidable_frac  ·  eqp[2~3] prev_prod · prev_oper",
        "lines": [
            "2332  eqp_local = np.zeros(4, dtype=np.float32)",
            "2333  current_eqp_id = self._current_eqp",
            "2334  if current_eqp_id and current_eqp_id in self.eqps:",
            "2335      eqp_obj = self.eqps[current_eqp_id]",
            "2336      eqp_local[2] = encode_normalized(eqp_obj.prev_prod, prod_idx, P)",
            "2337      eqp_local[3] = encode_normalized(eqp_obj.prev_oper, oper_idx, O)",
            "2342      max_avoidable = 0.0",
            "2343      for flat in self.get_feasible_ppk_oper(current_eqp_id):",
            "2346          if self._would_need_conversion(current_eqp_id, lc, tp):",
            "2347              eqp_local[0] = 1.0",
            "2353      eqp_local[1] = max_avoidable",
        ],
        "calc": [
            "EQP002.prev_prod=PPK002, prev_oper=OPER001 → prev_prod=0/(10-1)=0.0, prev_oper=0/(3-1)=0.0",
            "EQP002 feasible 중 (PPK001,OPER002) 전환 필요 → eqp[0]=1.0, avoidable_frac=0.619",
        ],
        "result": "needs_conversion=1.0 / avoidable_frac=0.619 / prev_prod=0.0 / prev_oper=0.0",
        "note": "prev_prod/prev_oper는 same_setup 보상(현재 EQP 기준)과 정렬 — 구 ch5 same_ppk(전역) 대체.",
    },
    {
        "group": "현재 설비 (EQP local) — avoidable_frac 상세",
        "title": "eqp[1] avoidable_frac (α)",
        "lines": [
            "2348              av = self._conversion_avoidable_fraction(",
            "2349                  current_eqp_id, ppk_f, oper_f, lc, tp,",
            "2350              )",
            "2352                  max_avoidable = av",
            "2353      eqp_local[1] = max_avoidable",
        ],
        "calc": [
    ...
    },
    {
        ...
        "lines": [
            "2374  context = np.zeros(4, dtype=np.float32)",
            "2375  if self._last_assigned:",
            "2376      la = self._last_assigned",
            "2377      context[0] = encode_normalized(la.get(\"plan_prod_key\"), prod_idx, P)",
            "2380      oper_guess = None",
            "2381      for ld in data.get(\"lots\", []):",
            "2382          if ld[\"lot_id\"] == la.get(\"lot_id\"):",
            "2383              oper_guess = ld.get(\"oper_id\")",
            "2384              break",
            "2385      context[1] = encode_normalized(oper_guess, oper_idx, O)",
            "2386      context[2] = encode_normalized(la.get(\"eqp_id\"), eqp_idx, len(data[\"eqp_ids\"]))",
            "2387      context[3] = encode_normalized(la.get(\"lot_cd\"), lot_cd_idx, max(len(lot_cd_idx), 1))",
        ],
        "calc": [
