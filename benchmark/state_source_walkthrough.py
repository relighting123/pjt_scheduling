...
MINI_A_DATASET = [
    ...
    ("직전 배정", "EQP003이 PPK001 · OPER002 · 세팅A를 배정받음"),
...
STATE_WALKTHROUGH = [
    ...
    {
        "group": "버킷 (Bucket) — (PPK001, OPER002), current_eqp=EQP002 기준",
        "title": "ch13 avoidable_frac (needs_conversion=1인 버킷에 한해 계산)",
        "lines": [
        ...
        "calc": [
            "EQP002가 고를 수 있는 feasible 버킷 중 (PPK001,OPER002)는 세팅 A 필요(EQP002는 B) → 전환 필요 → needs_conversion=1.0",
            "",
        ...
        "result": "needs_conversion (ch8) = 1.0   /   avoidable_frac (ch13) = 0.619",
        "note": "α=0.619 → 이 전환의 62%는 다른 설비가 대신 커버 가능했다는 뜻 "
                "→ 실제로 전환을 감행하면 avoidable_conversion = -8.0×0.619 ≈ -4.95 페널티. "
                "eqp_local의 max-reduce 스칼라가 아니라 버킷별로 세분화되어 있다.",
    },
]
