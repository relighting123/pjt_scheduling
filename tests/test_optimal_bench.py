...
#   - pipeline_wip_buildup_then_steady의 minprogress: PPK 진행률만 보고
#     공정 간 배분(전공정을 몰아 끝낸 뒤 후공정으로 넘어가야 함)은 보지
#     않아, 한 EQP(관측된 실행에서는 EQP003)가 OPER001↔OPER002를 왔다갔다
#     하다 마지막 OPER001 LOT을 t=[180,280)에야 마친다 — 그 LOT의 OPER002
#     (ST=40분)는 t=[280,320)이 되어 sim_end(280) 밖으로 밀려나 production
#     15/16(진짜 최적 16에 1건 미달). earliest_st는 이 케이스에서 정확히
#     최적(16/0)에 도달해 목록에 없다.
