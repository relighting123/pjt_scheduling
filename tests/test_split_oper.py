...
def test_build_split_rules_includes_oper_id():
    ...
    assert {"PPK001", "OPER001", "OPER002"} <= {
        r["PLAN_PROD_ATTR_VAL"] if k == "PLAN_PROD_ATTR_VAL" else r["OPER_ID"]
        for r in rules
        for k in ("PLAN_PROD_ATTR_VAL", "OPER_ID")
    }
    keys = {(r["PLAN_PROD_ATTR_VAL"], r["OPER_ID"]) for r in rules}
    assert keys == {("PPK001", "OPER001"), ("PPK001", "OPER002")}


def test_resolve_split_qty_by_oper():
    lookup = _build_split_lookup([
        {"PLAN_PROD_ATTR_VAL": "PPK001", "OPER_ID": "OPER001", "EQP_MODEL_CD": "A", "SPLIT_QTY": 3},
        {"PLAN_PROD_ATTR_VAL": "PPK001", "OPER_ID": "OPER002", "EQP_MODEL_CD": "A", "SPLIT_QTY": 5},
    ])
...
def test_sql_example_split_has_oper_id():
    ...
    assert "PLAN_PROD_ATTR_VAL" in text
