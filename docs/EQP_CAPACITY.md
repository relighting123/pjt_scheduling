...
`pipeline_wip_buildup_then_steady`가 유일한 회귀다(RL 16 → 11).

```
PPK001/OPER001: 재공 8매/8carrier → 재공상한 4대
PPK001/OPER002: 재공 0매/0carrier → 재공상한 0대 → 배분 0대
배분: OPER001에 4대 전부
```

증명된 최적은 **1대만 미리 OPER002로 전환**해 두는 비대칭 배분인데, 배분은 "지금 재공이
있는 곳"만 보므로 아직 비어 있는 후속 공정에 장비를 예약하지 못한다. 재공상한에 "이번
horizon 안에 상류에서 흘러올 예상량"을 반영하면 개선할 수 있다(미구현).
