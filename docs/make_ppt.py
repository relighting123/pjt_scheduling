...
# S/A/R 3열 요약
cols = [
    ("State  (관측)", "2,107차원 벡터", "전역 5 + 버킷 O×P×K×14 + 설비 2", STEEL),
...
txt(s, 0.9, 1.4, 11.6, 0.5, [[
    ...
    R("2,107차원 Box 벡터", 13.5, NAVY, True),
    R(" 입니다.  (O=3, P=10, K=5, 버킷 채널 F=14 기준)", 13, GRAY),
]])
# 공식 박스
box(s, 0.9, 2.05, 11.5, 0.66, NAVY)
txt(s, 0.9, 2.05, 11.5, 0.66, [[
    R("obs_dim = 5 (전역)  +  O×P×K×14 (버킷)  +  2 (현재 설비)  =  2,107", 14.5, WHITE, True)
]], align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
...
rows = [
    ...
    ("버킷 특징 (Bucket)", "2,100", "(OPER×PPK×MODEL) 격자의 14개 채널: 유효성·WIP비중·종료시각·처리율·셋업연속·takt·전환필요·공구여유·달성가능비·투영커버", False),
