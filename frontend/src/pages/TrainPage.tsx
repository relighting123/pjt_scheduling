...
export default function TrainPage({ config, summary, modelExists, onTrained, onRefresh }: Props) {
  ...
  const body = () => {
    const base = { total_timesteps:totalTs, learning_rate:lr, w_same_oper:wOper, w_idle_per_min:wIdle, train_budget_mode:budget, ...(budget==="episodes"?{n_episodes:nEps}:{}) };
      ...
      <aside className="ctrl-panel">
        <div className="card">
          ...
          <div className="train-slider">
            <div className="train-slider-label">동일 OPER 보너스 <span className="train-slider-val">{wOper.toFixed(1)}</span></div>
            <input type="range" min={0} max={5} step={0.5} value={wOper} onChange={e=>setWOper(Number(e.target.value))} disabled={loading} />
          </div>
          <div className="train-slider">
            <div className="train-slider-label">Idle 패널티/분 <span className="train-slider-val">{wIdle.toFixed(1)}</span></div>
            <input type="range" min={-3} max={3} step={0.1} value={wIdle} onChange={e=>setWIdle(Number(e.target.value))} disabled={loading} />
          </div>
        </div>
