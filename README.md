...
ST는 `(PPK, OPER, EQP_MODEL)`로 정해지므로(`abstract_arrange_map`) 같은 버킷 안에서도
장비마다 처리량이 다릅니다. `mean(1/ST) ≠ 1/mean(ST)`라 "대수 × 평균 ST"로 뭉치면 어긋나므로,
장비를 우선순위 순으로 한 대씩 붙이며 1/ST를 누적합니다. 대수가 정해지면 그 목록을 앞에서부터
잘라 모델별 대수로 바로 나뉩니다.

**2) 재공 적정성 사전 체크** — 대수만 맞춰도 그 대수를 먹일 재공이 없으면 의미가 없습니다.
n대를 붙이면 재공은 `재공 매수 / Σ(1/ST)`분 만에 마르므로, 그 소진 시간이
`CONFIG.env.capacity_min_run_minutes`(기본 60분 = 전환 시간) 이상인 최대 대수와
ready carrier 수(한 대는 최소 1 carrier) 중 작은 쪽까지만 정원으로 인정합니다
(여기서도 1단계와 같은 우선순위 목록을 한 대씩 누적합니다). 재공이 아예 없으면
구동 불가(`RUNNABLE_YN='N'`)로 남고 마스킹도 적용되지 않습니다.
...
**4) 라인 밸런스 (`capacity_line_balance`, 기본 True)** — 같은 PPK의 공정들은 직렬이라
최종 공정 out이 곧 제품 실적입니다. 앞 공정에 장비를 몰아줘도 뒤 공정이 못 받으면 재공만
쌓이므로, **최종 공정 계획에서 역산(pull)** 해 각 공정의 목표 out을 잡습니다
(`앞 공정 목표 = 뒤 공정 목표 − 뒤 공정이 이미 든 재공`). 모든 공정이 같은 목표 처리율을
갖게 되어 ST가 달라도 공정 간 capa(매/분)가 맞습니다 — 대수가 아니라 처리율이 맞는 것입니다.

**5) 적용 방식 (`capacity_alloc_mode`, 기본 `cap`)**

| 모드 | 동작 |
|------|------|
| `cap` | 버킷별 **상한**만 겁니다. 정원 합이 보유 장비를 넘을 수 있고, 실제 배분은 에이전트가 정합니다 |
| `allocate` | 보유 장비를 버킷에 **실제로 나눠줍니다**(부족률 water-filling). 배분표가 곧 마스크가 되어 합계까지 보유 장비와 맞는 계획이 나옵니다 |

`allocate`는 부족률(=`(잔여량 − 배분 커버)/잔여량`)이 가장 큰 버킷에 한 대씩 줍니다.
절대 부족분이 아니라 비율을 쓰는 이유는, 절대량으로는 잔여량이 큰 버킷 하나가 전부
독식하기 때문입니다. 동률일 때는 `PLAN_PRIORITY` → 처리 가능 장비가 적은(전용) 버킷 →
전환 불필요 → 잔여량 순입니다. 또 어떤 장비가 **다른 버킷의 유일한 후보**면 건너뛰어,
범용 버킷이 전용 버킷의 유일한 장비를 가져가 그 제품이 굶는 것을 막습니다.
계획 부족을 다 메우고 남는 장비는 재공이 남은 버킷에 더 배분합니다
(`capacity_alloc_fill_idle`, 기본 True — 유휴 장비는 순손실이므로).

효과(`benchmark/optimal` 33개 조합, 휴리스틱 3종 / 11개 케이스, RL `GENERALIZATION_V6`):

| | 휴리스틱 최적 도달 | 휴리스틱 전환 | RL 최적 도달 | RL 전환 |
|---|---|---|---|---|
| 마스킹 OFF | 18/33 | 111 | 3/11 | 62 |
| `cap` | 26/33 | 56 | 7/11 | 20 |
| `allocate` | **30/33** | **43** | **10/11** | **15** |

RL에서 효과가 더 큽니다 — 마스크가 행동 공간에서 "불필요한 전환을 유발하는 선택"을
구조적으로 지워주기 때문입니다. 유일한 회귀는 `pipeline_wip_buildup_then_steady`로,
아직 재공이 흘러오지 않은 후속 공정은 `wip_cap=0`이라 배분을 못 받아 미리 장비를
잡아두지 못합니다(자세한 내용은 아래 문서). 그래서 기본값은 여전히 꺼짐입니다.

> 예시 데이터로 계산식·마스킹 트레이스·적재 결과를 단계별로 따라가는 문서:
> **[docs/EQP_CAPACITY.md](docs/EQP_CAPACITY.md)**
