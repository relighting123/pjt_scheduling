...
| 테이블 | 용도 |
|--------|------|
| `RTS_RSLT_MAS` | 스케줄 결과 (매 회차 동일 FAC_ID 전체 교체 — 최신 결과만 유지, 다른 FAC_ID는 영향 없음). `PRODUCE_QTY`/`PLAN_QTY`는 RULE_TIMEKEY(07:00) 기준 당일 값이며 다음 07:00에 다음 날 계획으로 넘어간다(누적 아님). `PPK_OPER_REASON_CD/CTN`·`CARRIER_REASON_CD/CTN`에 각각 (PLAN_PROD_ATTR_VAL,OPER_ID) 버킷 선택 사유와 그 안에서 CARRIER_ID를 고른 사유가 코드+상세문구로 적재된다(`simulation/selection_reason.py`) |
