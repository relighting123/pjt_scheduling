...
│
├── data/
│   ├── loader.py           # JSON 로드·검증·DB fetch
│   ├── preprocessor.py     # JSON → env_data (abstract inventory, WIP init)
...
- **LOT_CD / TEMP**: `lot_master.json` (LOT별). EQP 직전 값과 다르면 **전환 60분** + tool `(LOT_CD, EQP_MODEL)` cap 검사.
...
경로: `external/dataset/{FAC_ID}/{train|test|infer}/{RULE_TIMEKEY}/input/`
...
| `lot_master.json` | LOT별 LOT_CD, TEMP |
...
리포트: `external/dataset/FAC_TAKT/takt_suite_report.json`
